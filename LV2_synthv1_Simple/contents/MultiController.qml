...
QQC2.Control {
    ...
    property list<Here.ControllerGroup> controllers
    ...
    Repeater {
        ...
        delegate: Item {
            ...
            property Here.ControllerGroup controller : modelData
            property var value : modelData.ctrl.value
            onValueChanged: {
                if(root.visible)
                    calculate()
            }
    ...
    Component {
        id: _controllerComponent
        Here.ControllerGroup {}
    }

    onControllersChanged: calculate()
    onControllersIdsChanged: {
        for(var i in root.controllersIds) {
            var id = root.controllersIds[i]
            var obj = _controllerComponent.createObject(root, ({'ctrl': zynqtgui.control.getAllControlAt(id)}))
            root.controllers.push(obj)
        }
    }
                ...
                contentItem: Column {
                    Repeater {
                        ...
                        delegate: Text {
                            width: parent.width
                            color: root.foregroundColor
                            text:  modelData.ctrl.title + " : " + modelData.ctrl.value.toFixed(2) + " | " + (modelData.ctrl.value/modelData.ctrl.max_value).toFixed(2)
                            font.pointSize: 6
                        }
                    }
                }
    ...
    function calculate() {
        ...
        var i = 0
        for (i; i < controllers.length; i++) {
            sumValue += controllers[i].ctrl.value
            console.log("Calculating medium controllers value", sumValue)
            fromValue += controllers[i].ctrl.value0
            toValue += controllers[i].ctrl.max_value
        }
    ...
    function setValue(value) {
        ...
        var i = 0
        for (i; i < controllers.length; i++) {
            controllers[i].ctrl.value = value
        }
